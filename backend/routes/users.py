...
from backend.utils.db.userdb import create_user, delete_user, update_user, verify_user
...
from backend.types.errors import CustomError, PasswordError, JSONError, ArgError
...
def error_handler(e):
    ...
    if isinstance(e, CustomError):
        ...
        return jsonify({"status": False, "error": message}), response
    return jsonify({"status": False, "error": "unexpected error"}), 500
...
def user_post_login(username, password, db_client):
    ...
    if verify_user(username, password, db_client):
        ...
        return jsonify({"status": True, "jwt": jwt}), 200
...
def user_handler(request, disable, db_client, s3_client):
    """
    User handler.
    ...
    try:
        ...
        if request.method == "POST":
            try:
                ...
                return user_post_login(args["username"], args["password"], db_client)
            except ArgError:
                args = parse_args(["token"], data)
                return user_post_token(args["token"])

        if disable:
            return jsonify({"status": False, "error": "disabled"}), 403
