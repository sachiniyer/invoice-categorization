...
import { register_user, login_user, verify_token, update_password, delete_user } from '../api/user';
...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const load_cookies = async () => {
        if (process.env.REACT_APP_USERNAME_COOKIE !== undefined &&
            ...
            if (username !== undefined &&
                ...
                verify_token(token, process.env.REACT_APP_HAPI).then((_) => {
                    setUser(userData);
                }).catch((_) => {
                    setUser(null);
                })
    ...
    const set_cookies = (userData: User) => {
        if (userData.username !== null &&
            ...
            Cookies.set(process.env.REACT_APP_USERNAME_COOKIE, userData.username, { sameSite: 'strict' });
            Cookies.set(process.env.REACT_APP_TOKEN_COOKIE, userData.token, { sameSite: 'strict' });
        }
    }
        ...
        try {
            if (userData !== null &&
                ...
                login_user(userData.username,
                    userData.password,
                    process.env.REACT_APP_HAPI).then((response) => {
    ...
    const register = (userData: User) => {
        try {
            if (userData !== null &&
                ...
                userData.password !== null &&
                process.env.REACT_APP_HAPI !== undefined) {
                register_user(userData.username,
                    userData.password,
                    process.env.REACT_APP_HAPI).then((_) => {
                        login(userData);
                        return true;
                    })
            }
    ...
    return (
        <UserContext.Provider value={{ user, loaded, login, register, update, del, logout }}>
            {children}
